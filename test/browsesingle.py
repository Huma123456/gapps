...
for i in browse:#get the list of all apps of a subcategory
    ...
    for j in app:
        ...
        if j['docid'] not in prevappid:   
            insert1 = "INSERT IGNORE INTO main(aid,cat,subcat) VALUES(%s, %s, %s)"
            val=(j['docid'],"APP",categ)
            cursor.execute(insert1,val)
            prevappid.append(j['docid'])
            
#****************Insert into other 2 tables******************
sql = "SELECT aid FROM main WHERE status ='%s'"
cursor.execute(sql % '0')
for row in cursor:
    print(row[0])
    #fetching the app details 
    j = server.details(row[0])
    # print(j.get('docid'),"\n",j.get('backendDocid'),"\n",j.get('title'),"\n",j.get('creator'),"\n",
    ...
    insert1 = "INSERT IGNORE INTO app_details(aid,backendDocid,title,creator,description,img_url,imgurl_convert,devName,version,install_size,dev_email,dev_web,num_downloads,pkg_name,htmlchng,upload_date,desc_short,comment_count) VALUES(%s, %s, %s,%s, %s, %s,%s, %s, %s,%s, %s, %s,%s, %s, %s,%s,%s, %s)"
    val=(row[0],j.get('backendDocid'),j.get('title'),j.get('creator'),j.get('descriptionHtml'),
    ...
    insert1 = "INSERT IGNORE INTO rating(aid,rate_type,star_rate,rate_cnt,one_star,two_star,three_star,four_star,five_star) VALUES(%s,%s, %s, %s,%s, %s, %s,%s, %s)"
    val=(row[0],j.get('aggregateRating').get('type'),j.get('aggregateRating').get('starRating'),
